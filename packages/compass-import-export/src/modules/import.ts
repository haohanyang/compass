...
export const startImport_ = (file: File): ImportThunkAction<Promise<void>> => {
  return async (
    dispatch,
    getState,
    {
    ...
    try {
      ...
      if (fileType === 'csv') {
        ...
        const resp = await atlasService.fetch('/upload-csv', {
          method: 'POST',
          body: formData,
        });
        ...
        const resp = await atlasService.fetch('/upload-json', {
          method: 'POST',
          body: formData,
        });
...
const loadTypes_ = (
  ...
  return async (
    dispatch,
    getState,
    { logger: { log, mongoLogId }, atlasService }
  ) => {
    const {
      fileName,
      delimiter,
    ...
    try {
      ...
      const resp = await atlasService.fetch('/analyze-csv-fields', {
        method: 'POST',
        body: formData,
      });
...
const loadCSVPreviewDocs_ = (file: File): ImportThunkAction<Promise<void>> => {
  return async (
    dispatch,
    getState,
    { logger: { log, mongoLogId }, atlasService }
    ...
    try {
      ...
      let resp = await atlasService.fetch('/list-csv-fields', {
        method: 'POST',
        body: formData,
      });
      ...
      await dispatch(loadTypes_(file, fields, values));
    } catch (err) {
      log.error(
        mongoLogId(1001000097),
        'Import',
...
export const selectImportFile = (
  ...
  return async (
    ...
    try {
      ...
      const resp = await atlasService.fetch('/guess-filetype', {
        method: 'POST',
        body: formData,
      });
      ...
      // type selects
      if (detected.type === 'csv') {
        await dispatch(loadCSVPreviewDocs_(file));
      }
    } catch (err: any) {
      log.info(
        mongoLogId(1_001_000_189),
